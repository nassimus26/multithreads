...
import java.util.concurrent.atomic.AtomicInteger;
 ...
 public class AkkaLinkDataCollector extends XmlNodeData<R151ExpectedKeysEnum, byte[]> {
     ...
     private void writeResult(XmlNode node) {
         ...
         res[res.length-2]='\r';
         res[res.length-1]='\n';
         ((AkkaLinkyFile)node.getNodeHandler().getProcessGeneric()).getProcessChunkEmitter().tell(new BytesRes(res), null);
     }
